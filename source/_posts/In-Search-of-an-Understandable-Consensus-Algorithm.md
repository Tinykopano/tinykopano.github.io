...
第三个问题是，被移除的服务器（那些不在C<sub>new</sub>中的服务器）会扰乱集群。这些服务器不会收到心跳，所以它们会超时并开始新的选举。然后他们会发送带有新任期编号的RequestVote RPCs，这将导致当前的leader恢复到追随者状态。一个新的leader最终将被选出，但被移除的服务器将再次超时，这个过程将重复，导致可用性差。
